      ...
      <div class="row">
        ...
        <div class="card card-outline card-secondary">
          <div class="card-header">
            <b style="text-align: center;">All Convicts</b>
          </div>
            ...
            <div class="table-responsive">
              <table class="table m-0 table-hover" id="home_data">
               <!-- <colgroup>
                ...
                if($_SESSION['login_type'] == 3 || $_SESSION['login_type'] == 2 || $_SESSION['login_type'] == 4){
                 ...
                 $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, r.name AS region_name, s.name AS station_name FROM prisoners p INNER JOIN regions r ON r.id = p.regions_id INNER JOIN stations s ON s.id = p.stations_id  WHERE p.stations_id = $stations_id AND p.status = 1 ORDER BY prisoners_no ASC");
                  
                }elseif($_SESSION['login_type'] == 5){

                  $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, r.name AS region_name, s.name AS station_name FROM prisoners p INNER JOIN regions r ON r.id = p.regions_id INNER JOIN stations s ON s.id = p.stations_id  WHERE p.regions_id = $regions_id AND p.status = 1 ORDER BY prisoners_no ASC");

                }elseif($_SESSION['login_type'] == 1){

                 $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, r.name AS region_name, s.name AS station_name FROM prisoners p INNER JOIN regions r ON r.id = p.regions_id INNER JOIN stations s ON s.id = p.stations_id  WHERE p.status = 1 ORDER BY prisoners_no ASC"); 
                }
              ...
              <button type="button" class="btn btn-default btn-sm btn-flat border-secondary wave-effect text-info dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
                        ...
                        <div class="dropdown-menu" style="">
                          ...
                          <a class="dropdown-item view_prison_data" href="#&id=<?php echo $row['id'] ?>" data-id="<?php echo $row['id'] ?>">Medical History</a>
                          <div class="dropdown-divider"></div>
