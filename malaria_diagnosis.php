...
?>
 ...
 <input type="hidden" name="stations_id" id="stations_id" value="<?php echo isset($stations_id) ? $stations_id : '' ?>">
    ...
    <option <?php echo isset($testing) && $testing == 'Diagnosed Negative' ? 'selected' : '' ?>>Diagnosed Negative</option>
    <option <?php echo isset($testing) && $testing == 'Diagnosed Positive' ? 'selected' : '' ?>>Diagnosed Positive</option>
...
<script>
	$('#manage-project').submit(function(e){
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_malaria',
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.href = 'index.php?page=malaria_diagnosis'
					},2000)
				}
			}
		})
