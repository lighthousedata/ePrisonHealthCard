...
<div class="card card-outline card-secondary">
    <div class="card-body">
      ...
      <thead>
          ...
          $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, p.id AS pid, hiv.* FROM prisoners p INNER JOIN hiv_test hiv ON p.prisoners_no = hiv.prisoners_no WHERE p.stations_id = $stations_id AND hiv.eligible_for_test = 'Yes' ORDER BY hiv.visit_date DESC");
         if($qry->num_rows > 0){
