		...
		<div class="card-body">
			...
			<!--	<colgroup>
				...
				<thead>
							...
							<th style="color: #CD853F; font-size: 12px;">STI NAME</th>
					...
					$qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, p.id AS pid, s.*, s.id AS soid FROM prisoners p INNER JOIN stis_service s ON p.prisoners_no = s.prisoners_no WHERE p.stations_id = $stations_id ORDER BY s.service_date DESC");
         if($qry->num_rows > 0){
					...
					<tr>
						...
						<td>
							<p><?php echo ucwords($row['sti_type']) ?></p>
                    ...
                    <?php //} if($stats==0)  { ?>
                      ...
                      <?php //} ?>
						...
						<td class="text-center">
                            <a class="dropdown-item drug_dispensory" href="./index.php?page=STI_review&soid=<?php// echo $row['soid'] ?>"><button class="btn btn-primary btn-sm pull-right" ><i class="fa fa-pencil-alt"></i></button></a>
                </div>
            </td> -->
