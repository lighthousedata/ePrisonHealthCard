...
if (isset($_POST['save'])) {
  $name = $_POST['name'];
    $query=mysqli_query($conn,"SELECT * FROM death_cause
            ...
            if ($counter == 1) 
              { 
                ...
                <script type = "text/javascript">
                    alert("This Death Cause is Already Taken!");
                    window.location = "index.php?page=death_and_causes";
                </script>
...
elseif ($conn->query("INSERT INTO death_cause (stations_id, regions_id,name) values ('$_POST[stations_id]','$_POST[regions_id]','$name')")) {
...
<div class="row">
 ...
 <p style="color:#CD853F; text-align: center;">
               Prisoners Number</p>
                ...
                <?php endwhile; ?>
                    ...
                    <div class="col-md-4">
                     <p style="color:#CD853F; text-align: center;">
            Date: </p>
            <input type="date" name="date" value="<?php echo isset($date) ? date("Y-m-d",strtotime($date)) : '' ?>" required style="width:100%; border-radius:5px;">
               
                    </div>

            <div class="col-md-4">
                <p style="text-align:center;color:#CD853F;">Death Causes &nbsp;&nbsp; <button class="btn btn-primary btn-sm pull-right" data-toggle="modal" data-target="#addIn" style="margin-left: 2px; font-size:8px;"><i class="fa fa-plus fa-fw"> </i></button></p>
                         
                <select class="form-control form-control-sm select2" name="causes" id="causes" required>
                <option></option>
                <?php 
                $regimen = $conn->query("SELECT * FROM death_cause ORDER BY name ASC ");
                while($row= $regimen->fetch_assoc()):
                ?>
                <option value="<?php echo $row['name'] ?>" <?php echo isset($causes) && $causes == $causes ? "selected" : '' ?>><?php echo ucwords($row['name']) ?></option>
                <?php endwhile; ?>
              </select>    
                    </div>
    ...
    <tr>
         <td>
                <p style="font-size:18px;color:#CD853F; font-weight: bold; text-align:center;" class="control-label">Remarks</p>
...
<script>
    $('#manage-project').submit(function(e){
        ...
        start_load()
        $.ajax({
            url:'ajax.php?action=save_death_and_causes',
            ...
            success:function(resp){
                if(resp == 1){
                    ...
                    setTimeout(function(){
                        location.href = 'index.php?page=death_and_causes'
                    },2000)
                }
            }
        })
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Add New Death Cause</h4>
      </div>
...
?>
 ...
 <input type="hidden" name="stations_id" id="stations_id" value="<?php echo isset($stations_id) ? $stations_id : '' ?>">
           <div class="form-group">
            <label for="some" class="col-form-label" style="color:#CD853F;">Death Cause</label>
