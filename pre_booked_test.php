...
if ($conn->query("INSERT INTO lab_tests (prisoners_no, stations_id, regions_id, screening, test_type, results, remarks, service_date) values ('$_POST[prisoners_no]','$_POST[stations_id]','$_POST[regions_id]','$_POST[screening]','$_POST[test_type]','$_POST[results]','$_POST[remarks]','$_POST[service_date]')")) {
  ...
  //$qry = $conn->query("SELECT MAX(id) FROM lab_tests");
  $id = mysqli_insert_id($conn);
  $conn->query("UPDATE book_lab_test SET status = 0, lab_test_id = '$id'  WHERE id = '$_POST[bid]'");

  echo '<script type="text/javascript">
                alert("Data Saved Successfully!");
                window.location = "index.php?page=pre_booked_lab_test_list";
                </script>';
   }else{
         echo '<script type="text/javascript">
...
if(isset($_GET['bid'])){
  ...
  // $prisoners_no = $_GET['pid'];
  
  $qry = $conn->query("SELECT * FROM book_lab_test where id = ".$_GET['bid'])->fetch_array();
  foreach($qry as $k => $v){
    $$k = $v;
  }
}
?>
 ...
 <input type="hidden" name="bid" value="<?php echo isset($id) ? $id : '' ?>">
...
<div class="row">
 <div class="col-md-4">
  <div class="card card-outline card-secondary">
  <div class="card-body">
    <p style="font-weight:bold; color:#CD853F; text-align:center;">Type of Test</p><hr>
<div class="form-group">
         <input type="text" class="form-control form-control-sm" name="test_type" value="<?php echo $test_type; ?>"  readonly  required>
        </div>
  </div>
  </div>
 </div>
...
<div class="card-footer border-top border-secondary">
        <div class="d-flex w-100 justify-content-center align-items-center">
          ...
          <button class="btn btn-flat  bg-gradient-primary mx-2" type="submit" name="save">Save </button>
...
<script>
  $('#manage-project').submit(function(e){
    ...
    start_load()
    $.ajax({
      url:'ajax.php?action=save_lab_tests_booked',
