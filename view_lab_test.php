		...
		<div class="card-body">
			...
			<!--	<colgroup>
				...
				<thead>
					...
					$qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, p.id AS pid, l.* FROM prisoners p INNER JOIN lab_tests l ON p.prisoners_no = l.prisoners_no WHERE p.stations_id = $stations_id ORDER BY l.service_date DESC");
         if($qry->num_rows > 0){
