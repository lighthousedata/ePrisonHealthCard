              ...
              <table class="table m-0 table-hover" id="home_data">
               <!-- <colgroup>
                ...
                if($_SESSION['login_type'] == 3 || $_SESSION['login_type'] == 2 || $_SESSION['login_type'] == 4){
                 ...
                 $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, r.name AS region_name, s.name AS station_name FROM prisoners p INNER JOIN regions r ON r.id = p.regions_id INNER JOIN stations s ON s.id = p.stations_id  WHERE p.stations_id = $stations_id AND p.status = 2 ORDER BY prisoners_no ASC");
                  
                }elseif($_SESSION['login_type'] == 5){

                  $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, r.name AS region_name, s.name AS station_name FROM prisoners p INNER JOIN regions r ON r.id = p.regions_id INNER JOIN stations s ON s.id = p.stations_id  WHERE p.regions_id = $regions_id AND p.status = 2 ORDER BY prisoners_no ASC");

                }elseif($_SESSION['login_type'] == 1){

                 $qry = $conn->query("SELECT p.*, concat(p.fname, ' ', p.lname) AS pname, r.name AS region_name, s.name AS station_name FROM prisoners p INNER JOIN regions r ON r.id = p.regions_id INNER JOIN stations s ON s.id = p.stations_id  WHERE p.status = 2 ORDER BY prisoners_no ASC"); 
                }
